# EFTFit
This repository holds the custom files needed to run an EFT fit using topcoffea datacards.
...
- Run `combineCards.py` to merge them all into one txt file. **DO NOT** merge multiple variables for the **same** channel, as this would artificially double the statistics!
  ...
  - TOP-22-006 selection (old method): `combineCards.py ttx_multileptons-{2,4}*lj0pt.txt ttx_multileptons-3l_{p,m}_offZ*lj0pt.txt ttx_multileptons-3l_onZ_1b_*ptz.txt ttx_multileptons-3l_onZ_2b_{4,5}j*ptz.txt ttx_multileptons-3l_onZ_2b_{2,3}j*lj0pt.txt > combinedcard.txt`
  - TOP-22-006 selection (new method): The latest tools should produce the correct lj0pt or ptz datacards for the corresponding categories. Therefore, you can simply run: `combineCards.py ttx_multileptons-*.txt > combinedcard.txt`
...
# Making postfit
Note: If you are making Asimov data postfit, the best way to approach this is to simply create a separate Asimov workspace with only Asimov datacards and perform the exact same steps in this section. (For topeft analysis group, turn off `--unblind` option to obtain Asimov datacards)
